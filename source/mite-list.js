...
Examples:
  ...
  export all time-entries from the current month as csv:
    mite list last_week --json --columns user,id | jq -rM '.[] | @csv'
  ...
  Create PDF with time entries from a specific project for the last month
    NO_COLOR=1 mite list last-month --project-id 1234 --columns date,service,note,duration --json | jq -rM '.[] | @csv' | npx csv2md
`)
  .parse();
